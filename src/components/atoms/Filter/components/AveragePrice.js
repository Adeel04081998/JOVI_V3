...
export default ({ data = {}, filterType = "", filterTypeStyle, colors, onPress, selectedFilter = {}, scrollEnabled = true, activeFilterBy = null }) => {
    // console.log("if data average=>>", data);
    // console.log("if selectedFilte  avergr", selectedFilter);

    const renderFilterByUi = (x, i) => {
        const isActive = activeFilterBy === i;
        let borderColor = isActive ? "#F3B8B4" : "#C1C1C1"
        let color = isActive ? "#F94E41" : "black"
        return (
            <View>
                <TouchableOpacity style={{ width: x.image ? null : 103, height: 29, borderWidth: 0.8, flexDirection: 'row', justifyContent: x.image ? "flex-start" : "center", alignItems: 'center', alignContent: 'center', marginLeft: i === 0 ? 0 : (x.image ? 9 : 10), borderRadius: 5, borderColor: borderColor, backgroundColor: "white", paddingHorizontal: 5, }}
                    onPress={() => { onPress(i) }}  >
